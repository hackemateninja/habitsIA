...
export function asyncForgot(body: {mail: string}){
	return async (dispatch: ActionCreator<any>) => {
		const response = await HTTP('POST', body,'changePass');
		dispatch(forgot(response.data, response.status, response.succes, response.error, body.mail));
	}
}
...
export function asyncGetCompany (body: object) {
	return async (dispatch: ActionCreator<any>) => {
		const response =  await HTTP('POST', body, 'getCompany');
		let areas: string[] = [];
		if (response.company){
...
export function asyncLogin (body: object)  {
	return async (dispatch: ActionCreator<any>) => {
		const response = await HTTP('POST', body, 'login' );
		if (response.token){
			const responseStringy = JSON.stringify(response);
			await AsyncStorage.setItem('@login', responseStringy).then(()=>{
				dispatch(login(true, response.user, response.token, response.message.title));
			})
		}else {
...
export function asyncRegisterPersonal (body: object) {
	return async(dispatch: ActionCreator<any>) => {
		const response = await HTTP('POST', body, 'saveUser')
		if (response.user){
			dispatch(
				registerPersonal(
...
export function asyncReset (body: object){
	return async (dispatch: ActionCreator<any>) => {
		HTTP('PUT', body, 'user/updatePassword')
			.then(e => {
