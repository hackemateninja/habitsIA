...
const RegisterPersonalInfo = ({navigation, theme, auth, register, clear}: any)=>{
...
//verifica y valida que los campos sean correctos para poder activar el boton
	useEffect(()=>{
		...
		if (validName && validMail && validPassword && validPasswordVerify && toggleCheckBox){
			setDisable(false);
			Keyboard.dismiss();
		}else {
			setDisable(true);
	...
	return(
		<GradientContainer
			...
			<BottomAuthScreenContainer
				...
				<Input
					value={mail}
					onChange={(value:string)=>{setMail(value)}}
				...
				<Input
					value={passVerify}
					onChange={(value:string)=>{setPassVerify(value)}}
					type='password'
...
export default connect(mapStateToProps, mapDispatchToProps)(RegisterPersonalInfo)
